...
fs.createReadStream('../server/data/microtrendData.csv')
.pipe(csvparser())
.on('data', function(data){
    try {
        sitelist.push(data.new_description)
...
.on('end',function(){
    ...
    // sitelist=fixUtf8(sitelist);
    sitelist = iconv.encode(sitelist, 'utf8');
    // console.log(sitelist)
    fs.writeFile('LOLOL.csv', sitelist, 'utf8', (err) => {
        if (err) throw err;
        ...
        request(url, (function(site) { return function(err, resp, body) {
            ...
            if (progress===0) {
                ...
                try {
                    ...
                    // console.log(csv)
                    fs.writeFile('LOLOL.csv', csv, 'latin1', (err) => {  
                        if (err) throw err;
                        console.log('CSV written to file');
                    });
                } catch (err) {
